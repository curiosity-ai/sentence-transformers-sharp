...
public sealed class VectorCache
{
    ...
    /// <summary>
    /// Encodes <paramref name="sentences"/> through <paramref name="encode"/>, but first satisfies any
    /// inputs already present in the cache (keyed by <c>Hash128()</c>) and only forwards the cache misses
    /// to <paramref name="encode"/>. Newly produced vectors are stored back into the cache. The returned
    /// array preserves the order of <paramref name="sentences"/>.
    /// </summary>
    /// <param name="sentences">Texts to embed.</param>
    /// <param name="encode">Delegate that encodes a batch of (cache-missed) sentences into vectors.</param>
    /// <param name="cancellationToken">Cancellation token forwarded to <paramref name="encode"/>.</param>
    public async Task<float[][]> EncodeWithCacheAsync(string[] sentences, Func<string[], CancellationToken, Task<float[][]>> encode, CancellationToken cancellationToken = default)
    {
        ...
        for (int i = 0; i < sentences.Length; i++)
        {
            ...
            if (TryGet(keys[i], out var cached))
            {
                results[i] = cached;
            }
            else
        ...
        var encoded = await encode(toEncode, cancellationToken).ConfigureAwait(false);

        for (int i = 0; i < misses.Count; i++)
        {
            var index = misses[i];
            results[index] = encoded[i];
            Set(keys[index], encoded[i]);
        }
